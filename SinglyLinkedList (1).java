...
public  class SinglyLinkedList<E> extends AbstractList<E> implements List<E>
{
  ...
  public void add (int ix, E value) {
    Entry<E> focus = head;
    Entry<E> parent = null;
    for (int i = ix; ix > 0; ix--) {
      parent = focus;
      focus = head.next;
    }
    final Entry<E> newFocus = new Entry<>(value, focus);
    if (parent != null) {
      parent.next = newFocus;
    } else {
      head = newFocus;
    }
  }
  ...
  public boolean add(E value) {
	  ...
	  Entry<E> newEntry = new Entry<E>(value, head);
	  head = newEntry;
	  return true;
  }
  ...
  {
    Entry<E> newEntry = new Entry<E>(element, head);
    head = newEntry;     
  } // method addToFront
